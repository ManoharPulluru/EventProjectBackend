...
const UserLoginForm = () => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post('https://eventbackend-2hj4.onrender.com/usersignin', formData)
      .then((response) => {
        let pass = response.data;
        console.log(pass)
        if(pass.message === 'success'){
          alert('Login Successful');
         navigate(`/userwelcome/${pass.userdata.contact}`)
        }else{
    ...
    <div className='form-parent'>
      <div className='form-head'>
        <div className='vendor-heading'>
          <Link to='/vendorsignin' className='no-underline'>
            Vendor
          </Link>
        </div>
        <div style={{ color: '#4E94F4' }} className='user-heading'>
          User
        </div>
      </div>
      ...
      <div className='form-foot'>
        <div className='create-ac'>
          <Link to='/usersignup' className='no-underline'>
            Create Account
          </Link>
        </div>
